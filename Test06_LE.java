...
Q3. Syntax for creating LE?
	- A LE has two parts 
			1. parameters and
			2. body with logic
	           connected an labmda operator [->]

	- (parameter) -> {logic}; 
	 
	- this '->' is called lambda operator
	- hence this expression is called labda expression
	
	- A lamda operator '->' is a binary operator, 
...
Q5) What is the internal name, parameters, return type and exception of a LE?
	...
	- all above things are derived from the FI 
	  to which this LE is assigned.
	 
Q6) When LE does not have name, how can we execute its logic?
	- we must call LE by usign FI method name
	  by passing arguments according to FI method paramters
...
Q8) LE Short-cut syntaxes?
	...
	FI fi = (a) -> stmt;   	//2. for parameter type is not required
	FI fi =  a 	-> stmt;   	//3. for one parameter () is not required
	FI fi = (a, b) 	-> stmt;//4. for zero multiple parameters () is required
	FI fi = (a, b) 	-> 50;	//5. we can return value direclty, 
	                        //   { } and return kw not required
